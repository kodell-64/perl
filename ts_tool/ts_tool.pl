...
$secondaryargs{"g"} = "t" if($graphoption eq "pcrs-per-second");
$secondaryargs{"g"} = "t" if($graphoption eq "pcr-accuracy");

$argsdescription{"a"} = $mode eq "a" ? "source ATSC params" : "source address";
...
$argsdescription{"c"} = "capture filename, ex. -r foo.ts";
...
$graphoptions{"pcrs-per-second"} = "[PCR/s] ex. -g pcrs-per-second";
$graphoptions{"pcr-accuracy"} = "[PCR Accuracy] ex. -g pcr-accuracy";
$graphoptions{"bitrate"} = "[Bitrate] bitrate, ex. -g bitrate";

if(scalar(@cmdlineargs) == 0 || $version)
{
    print "\n\nPerl Transport Stream tool";
    print "\nVersion: $VERSION";
...
&analyzecanadastream() if($mode eq "c");
&analyzefile($file) if($opt{r});

sub test
{
    print "\nTest modules for ts_tool.pl";
    ...
    while(1)
    {
        ...
        if(scalar(@ready))
        {
            ...
            for(my $p=0; $p<scalar(@bytes); $p+=11, $packet += 188)#88)
            {
                ...
                if($af)
                {
                    ...
                    if($afl > 1)
                    {
                        ...
                        if(($af == 2 || $af == 3) && $pcrflag && $pid == $pcrpid)
                        {
                            ...
                            $pcrbase = $pcrbase << 1;
                            $pcrbase += $pcrb;
                            ++$stream->{"$pid"}->{"pcrsthissecond"};
                            $stream->{"$pid"}->{"pcrflag"} = $pcrflag;
                            my @times = gettimeofday();
                            my $t1 = $times[1];
                            my $a = $stream->{"$pid"}->{"lastpcr"};
                            my $b = $t1;
                            if($t1  < $stream->{"$pid"}->{"lastpcr"})
                            {
                                $b = $stream->{"$pid"}->{"lastpcr"};
                                $a = $t1;
                            }

                            $stream->{"$pid"}->{"lastpcrdelta"} = sprintf("%f", $b - $a );
                            #$stream->{"$pid"}->{"lastpcrdelta"} = sprintf("%d", ($b - $a)/1000);
                            if(0)#$stream->{"$pid"}->{"lastpcrdelta"} > 40)
                            {
                                print $stream->{"$pid"}->{"lastpcrdelta"}." : PCR pid $pid\n";
                            }
                            $stream->{"$pid"}->{"lastpcr"} = $t1;



                            if($graphoption eq "pcr-bitrate" 
                               || $graphoption eq "pcrs-per-second"
                               || $graphoption eq "pcr-accuracy")
                            {
                                ...
                                my $pcrdelta = ($pcrbase - $stream->{"$pid"}->{"lasttruepcr"});
                                ...
                                $avg = $pcrdelta / $pcrpidpackets;
                                #$avg /= $pcrs;

                                my $expected = $avg * $pcrpidpackets;
                                my $actual = $pcrdelta - $expected;
                                $actual = $actual * 10**13;
                                push @pcraccuracies, $actual;#stream->{"$pid"}->{"lastpcrdelta"};
                                shift @pcraccuracies if(@pcraccuracies > 90);
                            ...
                            $stream->{"$pid"}->{"lasttruepcr"} = $pcrbase;
                        }
                        else
                        {
                ...
                $stream->{"$pid"}->{"packetsthissecond"} += 1;
            ...
            if($dstaddr && $dstport)
            {
                ...
                if($shortmsg && 0)
                {
                    $d->mcast_send($shortmsg, "$dstaddr:$dstport");
                }
            }
            ...
            if($elapsed > $timeinterval && !$paused)
            {
                ...
                my @eshlabels = ("pid    ", "packets       ", "[p/s]    ", "Mbps      ", "CC errors", "UDP packet/dev.",
                                 "Late", "AF", "AFL ", "PCR/s", "PCRD ", "PCR-Base   ", "PCR-Rollover", "SC", "SC\%");
                my @eshcols = (0);
                my $l;
                ...
                foreach my $pid(@pids)
                {
                    ...
                    push @fields, "[".$stream->{"$pid"}->{"pcrsthissecond"}."]";
                    push @fields, "[".$stream->{"$pid"}->{"lastpcrdelta"}."]";
                ...
                foreach my $pid(@pids)
                {
                    ...
                    $stream->{$pid}->{"packetsthissecond"} = 0;#$stream->{$pid}->{"bytesthissecond"} / 188;
                    $stream->{$pid}->{"bytesthissecond"} = 0;
                    $stream->{$pid}->{"pcrsthissecond"} = 0;
                }
                ...
                if($graphoption)
                {
                    ...
                    elsif($graphoption eq "pcrs-per-second")
                    {
                        ++$curx;
                        addstr($curx++,0,"PCR/s -> ".$pcrs);
                        push @apcrs, $pcrs;
                        shift @apcrs if(@apcrs > 90);
                        $pcrs = 0;
                        addstr($curx++,0, &graph($height, 10, $#apcrs, \@apcrs, ""));
                    }
                    elsif($graphoption eq "pcr-accuracy")
                    {
                        ++$curx;
                        addstr($curx++,0, &graph($height, 10, $#pcraccuracies, \@pcraccuracies, ""));
                    }
...
##
##
## elementalanalyzemdi-df                m           ##########################################
##
## Simple code example for Elemental to use to analyze MDI-DF
## Korey O'Dell July 2, 2010
##
###############################################################################################

sub elementalanalyzestream
{
    # create a new UDP socket ready to read datagrams 
    my $s = IO::Socket::Multicast->new(LocalAddr=>$srcaddr,
                                       LocalPort=>$srcport, 
...
sub analyzecanadastream
{
    ...
    while(1)
    {
        ...
        while($elapsed < $timeinterval)
        {
            ...
            {
                $s->recv($srcmsg, 188*20);
            }
            print "\nread ".length($srcmsg)." bytes";
            #my @bytes =  unpack "(C18 x170)*", $srcmsg;
            my @bytes =  unpack "(C)*", $srcmsg; 
